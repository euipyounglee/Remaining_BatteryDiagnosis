...
namespace BaseLib.WebSocket
{
    ...
    public enum PayloadDataType
    {
        ...
        public class WebSocketController
        {
                        ...
                        case PayloadDataType.Text:
                            {
                                ...
                                //수신된 메세지
                                Console.WriteLine("Recive Text-OK:" + strResponseValue);
                                Debug.WriteLine(strResponseValue);
            ...
            public int SendData(string strData)
            {
                ...
                if (null != messageStream)
                {
                    ...
                    return SendData(data, PayloadDataType.Text);
                    //return true;
                }
        ...
        public int SendData(byte[] data,  int opcode)
        {
           return SendData(data, (PayloadDataType)opcode);
        }

            public int SendData(byte[] data, PayloadDataType opcode)
            {
                 int bResult = 0;// false;
                byte[] sendByteData;
